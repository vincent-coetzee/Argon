...
//        }
    ...
    private var editedNode: IDESourceFileNode!
    ...
    public func beginEditing(node: IDENode?)
        ...
        if let sourceNode = node as? IDESourceFileNode
            {
            self.editedNode = sourceNode
            self.string = sourceNode.source
            let attributes: [NSAttributedString.Key:Any] = [.foregroundColor: StyleTheme.shared.color(for: .colorEditorText)]
            self.textStorage?.setAttributes(attributes,range: NSRange(location: 0,length: sourceNode.source.count))
...
//            }
    ...
    @objc func textDidChange(_ sender: Any?)
        {
        self.editedNode.setSource(self.string)
        self.updateFromSource()
        }
        
    private func updateFromSource()
        {
        let scanner = ArgonScanner(source: self.string)
