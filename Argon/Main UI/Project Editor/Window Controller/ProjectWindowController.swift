...
//            }
    ...
    @IBAction func handleMenuEvent(_ sender: Any?)
        ...
        switch(menuItem.identifierValue)
            {
            case("buildProject"):
                self.buildProject()
                return
...
//            return
//            }
        self.inspectorViewController.rootModule = compiler.rootModule
        let integer = ArgonModule.shared.lookupSymbol(atName: "integer")!
...
//        self.issueCountIconLabelView.textColorElement  = count > 0 ? .colorIssue : .colorToolbarText
    ...
    private func onToggleIssuesClicked(_ sender: Any?)
        {
